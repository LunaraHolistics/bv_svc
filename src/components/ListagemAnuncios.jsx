    ...
    <div className="p-4">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 px-2 py-0.5 text-[10px] font-medium rounded-full ${
          anuncio.status === 'Ativo' ? 'bg-emerald-100 text-emerald-700' :
          anuncio.status === 'Vendido' ? 'bg-amber-100 text-amber-700' :
  ...
  if (anuncios.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <Link
          to="/novo-anuncio"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors no-underline"
        >
          Criar Anúncio
        </Link>
      </div>
