...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotificacoes = async () => {
      ...
      // Filtrar apenas os que NÃO foram lidos
    ...
    // Escutar em tempo real se um novo aviso for publicado
  ...
  const marcarComoLido = async (avisoId) => {
    ...
    // Atualizar a lista removendo o que foi lido
          ...
          <div className="h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {!loading && user ? (
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative" ref={notifRef}>
                    <button 
                      ...
                      className="relative p-2.5 bg-gray-50 hover:bg-gray-100 rounded-xl transition cursor-pointer"
                    >
                    ...
                    {notifOpen && (
                      <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden z-50">
                        <div className="p-3 bg-gray-50 border-b font-semibold text-sm text-gray-700 flex justify-between items-center">
                          ...
                          {naoLidas.length > 0 && (
                            <button onClick={marcarTodasComoLidas} className="text-xs text-emerald-600 font-medium hover:underline cursor-pointer">
                          ...
                          {naoLidas.length === 0 ? (
                            ...
                            naoLidas.map(aviso => (
                              <div key={aviso.id} className="p-3 border-b hover:bg-gray-50">
                                ...
                                <button onClick={() => marcarComoLido(aviso.id)} className="text-xs text-blue-500 mt-2 hover:underline cursor-pointer">
                                  Marcar como lido
                                </button>
                              </div>
                            ))
                  ...
                  <div className="relative" ref={dropdownRef}>
                    ...
                    {menuOpen && (
                      <div className="absolute right-0 mt-2 w-52 bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
                        ...
                        {/* Link secreto para Admin do Condomínio (aparece só pra quem tem permissão) */}
                        {(perfil?.is_admin_condominio) && (
                          <button onClick={() => { setMenuOpen(false); navigate('/adm_bv') }} className="w-full text-left px-4 py-3 text-sm hover:bg-orange-50 text-orange-600 cursor-pointer font-medium">
                            🏢 Painel Adm. Condomínio
                          </button>
                        )}
                ...
                <button onClick={() => navigate('/login')} className="px-5 py-2.5 border border-emerald-200 text-emerald-700 rounded-xl text-sm font-semibold hover:bg-emerald-50 transition cursor-pointer">
                  Entrar
                </button>
              )}
            </div>
      ...
      <nav className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-50 w-[94%] max-w-md pb-[env(safe-area-inset-bottom)]">
        <div className="bg-white/95 backdrop-blur-xl border border-gray-200 shadow-2xl rounded-3xl px-1.5 py-2">
          <div className="flex items-center justify-between">
            ...
            <NavLink to="/novo-anuncio" className="w-12 h-12 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-500 flex items-center justify-center text-white text-xl font-bold shadow-xl -mt-7 no-underline">
              +
            </NavLink>
            ...
            {navItems.slice(2, 4).map((item) => {
              const active = isActive(item.to)
              return (
                <NavLink key={item.to} to={item.to} className="flex flex-col items-center flex-1 no-underline">
                  <span className={`text-lg ${active ? 'scale-110' : 'opacity-70'} transition-transform`}>{item.emoji}</span>
                  <span className={`text-[10px] mt-0.5 ${active ? 'text-emerald-600 font-semibold' : 'text-gray-400'}`}>{item.label}</span>
                </NavLink>
              )
            ...
            {/* PERFIL MOBILE COM SINO */}
            <div className="flex flex-col items-center flex-1 relative">
              {user && naoLidas.length > 0 && (
                <span className="absolute -top-1 right-2 w-4 h-4 bg-red-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center z-20">
                  {naoLidas.length}
                </span>
              )}
              <button
