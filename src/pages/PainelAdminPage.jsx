      ...
      <div className="max-w-6xl mx-auto mb-8">
        <div className="flex items-center justify-between bg-red-600 text-white p-6 rounded-2xl shadow-lg">
          <div>
            ...
            <p className="text-red-100 text-sm mt-1">Acesso restrito. Logado como: {user?.email}</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={handleLogout} className="px-4 py-2 bg-white text-red-600 rounded-xl text-sm font-bold hover:bg-red-50 transition cursor-pointer">
              Sair
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 bg-white rounded-2xl border border-gray-200 p-6 min-h-[500px]">
          {loading ? (
            ...
            <div>
              ...
              {servicos.length === 0 ? <p className="text-gray-500">Nenhum serviço cadastrado.</p> : (
                <div className="space-y-3">
                  {servicos.map((ser) => (
                    <div key={ser.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 truncate">{ser.nome} {ser.nome_fantasia ? `- ${ser.nome_fantasia}` : ''}</p>
                        <p className="text-xs text-gray-500 truncate">{ser.categoria || 'Sem categoria'} • ID: {ser.user_id?.substring(0,8)}...</p>
                      </div>
                      <button onClick={() => deletarServico(ser.id)} className="ml-4 px-3 py-1.5 bg-red-50 text-red-600 text-xs font-bold rounded-lg hover:bg-red-100 transition cursor-pointer shrink-0">
                        EXCLUIR
                      </button>
                    </div>
                  ))}
            ...
            <div>
              ...
              {anuncios.length === 0 ? <p className="text-gray-500">Nenhum anúncio cadastrado.</p> : (
                <div className="space-y-3">
                  {anuncios.map((anun) => (
                    <div key={anun.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 truncate">{anun.titulo}</p>
                        <p className="text-xs text-gray-500 truncate">{anun.status || 'Ativo'} • ID: {anun.user_id?.substring(0,8)}...</p>
                      </div>
                      <button onClick={() => deletarAnuncio(anun.id)} className="ml-4 px-3 py-1.5 bg-red-50 text-red-600 text-xs font-bold rounded-lg hover:bg-red-100 transition cursor-pointer shrink-0">
                        EXCLUIR
                      </button>
                    </div>
                  ))}
