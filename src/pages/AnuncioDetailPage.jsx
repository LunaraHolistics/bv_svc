  ...
  if (error) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center">
        ...
        <Link
          to="/anuncios"
          className="px-6 py-3 bg-emerald-600 text-white rounded-2xl font-semibold hover:bg-emerald-700 transition no-underline"
        >
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="flex gap-1 overflow-x-auto p-2 bg-gray-50 snap-x snap-mandatory">
              {imagens.map((img, index) => (
                <img
                  ...
                  className="h-64 sm:h-80 flex-shrink-0 w-auto object-contain rounded-xl snap-start"
                />
              ))}
            </div>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className={`px-3 py-1 text-xs font-medium rounded-full ${statusCor}`}>
              {statusNormalizado}
            </span>
          ...
          <p className="text-gray-500 leading-relaxed whitespace-pre-line mb-6">
            {anuncio.descricao}
          </p>

          {/* Info */}
          <div className="border-t border-gray-100 pt-4 mb-4 space-y-2 text-sm text-gray-500">
            <p>
              Publicado em{' '}
              <span className="font-medium text-gray-700">
                {new Date(anuncio.created_at).toLocaleDateString('pt-BR')} às{' '}
                {new Date(anuncio.created_at).toLocaleTimeString('pt-BR', {
                  hour: '2-digit',
                  minute: '2-digit'
                })}
              </span>
          ...
          <div className="flex flex-wrap gap-3 pt-2 border-t border-gray-100">
            <a
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-green-500 text-white rounded-2xl text-sm font-medium hover:bg-green-600 transition no-underline"
            >
            ...
            {ehDono && (
              <button
                ...
                className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-2xl text-sm font-medium hover:bg-gray-200 transition no-underline cursor-pointer"
              >
                ✏️ Editar
              </button>
            )}
