...
const AdmBVPage = () => {
  ...
  const handlePublicar = async (e) => {
    ...
    const { error } = await supabase
      .from('avisos_admin')
      .insert({ titulo, mensagem, criado_por: user.id })
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="flex items-center justify-between bg-gradient-to-r from-amber-500 to-orange-500 text-white p-6 rounded-2xl shadow-lg">
          <div>
            ...
            <p className="text-amber-100 text-sm mt-1">Logado como: {user?.email} {perfil?.is_admin_condominio ? '(Super Admin BV)' : '(Master Dev)'}</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => navigate('/')} className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-xl text-sm font-medium transition cursor-pointer">
              Voltar ao Site
            </button>
            <button onClick={handleLogout} className="px-4 py-2 bg-white text-orange-600 rounded-xl text-sm font-bold hover:bg-orange-50 transition cursor-pointer">
              Sair
            </button>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto grid md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-2 bg-white rounded-2xl border border-gray-200 p-6 h-fit sticky top-8">
          ...
          <form onSubmit={handlePublicar} className="space-y-4">
            <input
              type="text"
              placeholder="Título do aviso (ex: Reunião de condôminos)"
              value={titulo}
              onChange={(e) => setTitulo(e.target.value)}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-400 outline-none"
            />
            <textarea
              placeholder="Escreva os detalhes do aviso para os moradores..."
              value={mensagem}
              onChange={(e) => setMensagem(e.target.value)}
              rows={5}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-400 outline-none resize-none"
            />
            <button
              type="submit"
              disabled={enviando}
              className="w-full py-3 bg-orange-500 text-white rounded-xl font-bold hover:bg-orange-600 transition disabled:opacity-50 cursor-pointer"
            >
        ...
        <div className="md:col-span-3 bg-white rounded-2xl border border-gray-200 p-6 min-h-[400px]">
          ...
          {loading ? (
            ...
            <p className="text-gray-400 text-center py-10">Nenhum aviso publicado ainda.</p>
          ) : (
                ...
                <div key={aviso.id} className={`p-4 border rounded-xl transition ${aviso.ativo ? 'border-green-200 bg-green-50/50' : 'border-gray-100 bg-gray-50 opacity-60'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${aviso.ativo ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-600'}`}>
                          {aviso.ativo ? 'VISÍVEL' : 'OCULTO'}
                        </span>
                      </div>
                  ...
                  <div className="flex gap-2 mt-3 border-t pt-3">
                    <button 
                      ...
                      className="text-xs font-medium px-3 py-1 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition cursor-pointer"
                    >
                      {aviso.ativo ? 'Desativar' : 'Ativar'}
                    </button>
                    <button 
                      onClick={() => deletarAviso(aviso.id)} 
                      className="text-xs font-medium px-3 py-1 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition cursor-pointer"
                    >
                      Excluir
                    </button>
                  </div>
