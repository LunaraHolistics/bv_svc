...
const IndicacoesPage = () => {
  ...
  const mediaGeral = useMemo(() => {
    const avaliadas = indicacoes.filter(
      (i) => Number(i.avaliacao_media) > 0
    )

    if (!avaliadas.length) return '--'

    const total = avaliadas.reduce(
      (acc, item) => acc + Number(item.avaliacao_media),
      0
    )
    ...
    <div className="space-y-8">
        ...
        <div className="flex gap-2 overflow-x-auto mt-4 pb-1">
          {categoriasFiltro.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm whitespace-nowrap transition ${filtroCategoria === cat
      ...
      {loading ? (
        <div className="grid md:grid-cols-3 gap-5">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-64 bg-white rounded-3xl border animate-pulse"
            />
          ))}
        </div>
      ) : error ? (
        ...
        <div className="bg-white rounded-3xl border p-12 text-center">
          ...
          <p className="text-gray-500 mt-2">
            Tente outro filtro ou seja o primeiro a indicar alguém.
          </p>
        </div>
      ) : (
      ...
      <section className="bg-gradient-to-r from-slate-900 to-blue-900 text-white rounded-3xl p-8 text-center">
        <h2 className="text-2xl font-bold">
          Conhece um profissional excelente?
        </h2>

        <p className="text-blue-100 mt-2">
          Ajude a comunidade compartilhando boas experiências.
        </p>
